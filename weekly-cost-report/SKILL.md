...
2. Script scans all `.jsonl` files in `$OPENCLAW_SESSIONS_DIR (default `~/.openclaw/agents/main/sessions`)/`
...
- OpenClaw session logs at `$OPENCLAW_SESSIONS_DIR (default `~/.openclaw/agents/main/sessions`)/*.jsonl`
